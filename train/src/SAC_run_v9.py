...
def save(agent, name):
    print(agent.path)
    if os.path.isdir(agent.path+str(EPISODES[name])): shutil.rmtree(agent.path+str(EPISODES[name]))
    os.mkdir(agent.path+str(EPISODES[name])+"_s_rate "+str(SUCCESS_RATE))
    ckpt_path = os.path.join(agent.path+str(EPISODES[name])+"_s_rate "+str(SUCCESS_RATE), 'SAC.ckpt')
